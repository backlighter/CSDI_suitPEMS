...
class Traffic_Dataset(Dataset):
    def __init__(self,observed_values,values,observed_masks,gt_mask,isTest=False,eval_length=12):
        ...
        if isTest:  
            self.observed_values = values*gt_mask
...
def get_dataloader(true_datapath,miss_datapath,val_ratio,test_ratio, batch_size=16,eval_length=12):
    ...
    observed_masks = np.nan_to_num(true['mask'][:1000,:,0])
    gt_masks= np.nan_to_num(miss['mask'][:1000,:,0])
    
    observed_values = np.nan_to_num(miss['data'][:1000,:,0].astype(np.float32))

    true_data = np.nan_to_num(np.load(true_datapath)['data'][:1000,:,0].astype(np.float32))
